    ...
    <div v-show="!isLoading" class="main" :class="this.$store.state.showTopMenu== true ? 'collapsed ' : 'uncollapsed'">
      <div class="container-fluid p-4">
        ...
        <div class="row mb-3">
          <!-- Col -->
          <div class="col-6">
            <h1 class="page-title">კატეგორიები</h1>
          </div>
          <!-- End Col -->
          <!-- Col -->
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      ...
      axios.get(process.env.VUE_APP_BACKEND_URL+'/auth/Categories/LoadRecords' +'?page=' + page,{
        params: {
    ...
    deleteDataById(id){
      const token = JSON.parse(localStorage.getItem('user')).access_token;
      axios.delete(process.env.VUE_APP_BACKEND_URL+'/auth/Categories/delete/'+id, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
