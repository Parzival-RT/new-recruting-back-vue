    ...
    <div v-show="!isLoading" class="main" :class="this.$store.state.showTopMenu== true ? 'collapsed ' : 'uncollapsed'">
      <div class="container-fluid p-4">
        ...
        <div class="row mb-3">
          <!-- Col -->
          <div class="col-6">
            <h1 class="page-title">პოზიციები</h1>
          </div>
          <!-- End Col -->
          <!-- Col -->
          <div class="col-6 text-end">
            <router-link to="/AddPosition">
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center bg-light">
            ...
            <div class="input-group w-50">
              ...
              <button class="btn btn-outline-primary d-none" @click="getResults()" type="button" id="button-addon2"><i class="tio-search"></i></button>
            </div>
          </div>
          <div class="card-body">
            ...
            <div class="row">
              <div class="col-12 mt-2">
                ...
                <table class="table">
                  <tbody>
                  <tr v-for="post in searchHandler" :key="post.id">
                    <td>
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      ...
      axios.get(process.env.VUE_APP_BACKEND_URL+'/auth/position/LoadRecords' +'?page=' + page,{
        params: {
    ...
    deleteDataById(id){
      const token = JSON.parse(localStorage.getItem('user')).access_token;
      axios.delete(process.env.VUE_APP_BACKEND_URL+'/auth/position/delete/'+id, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
    ...
    EditDetail(id){
      this.$router.push({ name: 'positionEdit', params: { id: id  } })
    },
  },
  computed: {
    searchHandler() {
     ...
     /* return this.ResponseData.data.filter(elem=>{
        console.log(elem.name.includes(this.keyword));
        return;
        return elem.name.includes(this.keyword);
      })*/
