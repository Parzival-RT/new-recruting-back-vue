...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      ...
      axios.get(process.env.VUE_APP_BACKEND_URL+'/auth/sphere/LoadRecords' +'?page=' + page,{
        params: {
    ...
    deleteDataById(id){
      const token = JSON.parse(localStorage.getItem('user')).access_token;
      axios.delete(process.env.VUE_APP_BACKEND_URL+'/auth/sphere/delete/'+id, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  computed: {
    filter() {
      const searchResult = this.ResponseData.data
        // .filter(data => data.name.toLowerCase().includes(this.keyword.toLowerCase()));
      return searchResult
    }
  }
}
